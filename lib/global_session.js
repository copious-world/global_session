...
const {fix_path,make_process_identifier} = require('./utils')
...
class SessionCacheManager {
  ...
  async get(key) {    // token must have been returned by set () -> augmented_hash_token
    let value = await this._LRUManager.get(key)
    if ( !value && this.message_fowarding ) {
      ...
      this.message_fowarding.publish(WANT_SESSION_FOR_ID,this.conf.auth_path,message)
      return false
    }
    return value
  }
  ...
  set(key,value) {
    let augmented_hash_token = this._LRUManager.set(key,value)
    if ( this.section_manager && this.message_fowarding ) {
      ...
      // Tell all subscribers that a new session has been created
